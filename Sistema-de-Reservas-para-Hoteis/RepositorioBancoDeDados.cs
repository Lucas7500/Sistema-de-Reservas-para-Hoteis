...
namespace Sistema_de_Reservas_para_Hoteis
{
    internal class RepositorioBancoDeDados : IRepositorio
    {
        ...
        public void Criar(Reserva reserva)
        {
            using SqlConnection connection = new(connectionString);
            connection.Open();
            SqlCommand inserirReservaNaTabela = new($"INSERT INTO TabelaReservas (Nome, Cpf, Telefone, Idade, Sexo, CheckIn, CheckOut, PrecoEstadia, PagamentoEfetuado) VALUES ('{reserva.Nome}', '{reserva.Cpf}', '{reserva.Telefone}', '{reserva.Idade}', '{reserva.Sexo}', '{reserva.CheckIn.Date:dd-MM-yyy}', '{reserva.CheckOut.Date:dd-MM-yyy}', '{reserva.PrecoEstadia.ToString().Replace(',', '.')}', '{reserva.PagamentoEfetuado}')", connection);
